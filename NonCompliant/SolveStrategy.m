...
global AD maxSG alpha gamma beta rou DSR DSConfirmations;
...
for i = 2:(mid2Until(AD)+1) % process states in Phase 1
    ...
    if l1 == l2 % chain 2 is orphaned
        ...
        Rs{on2}(i, base) = a1;
        if l2 >= DSConfirmations
            Rs{on1}(i, base) = Rs{on1}(i, base)+DSR*(l2-DSConfirmations+1);
            Rs{on2}(i, base) = Rs{on2}(i, base)+DSR*(l2-DSConfirmations+1);
        end
    ...
    if l2 == AD % about to enter Phase 2, chain 1 is orphaned
        if P{on1}(i, baseP2) == 0 % have phase 2
            ...
            Rs{on2}(i, baseP2) = alphaInAG*(a2+1)+gammaInAG*a2;
        else % no phase 2
        ...
        if l1 >= DSConfirmations
            Rs{on1}(i, baseP2) = Rs{on1}(i, baseP2)+DSR*(l1-DSConfirmations+1);
            Rs{on2}(i, baseP2) = Rs{on2}(i, baseP2)+DSR*(l1-DSConfirmations+1);
        end
    else
...
if numOfStates == numOfStatesP1P2 % have phase 2
    for i = (mid2Until(AD)+2):numOfStatesP1P2 % process states in P2
        ...
        if l2 == 0 % before the blockchain is forked
            ...
            if l1 == l2 % chain 2 is orphaned
                ...
                if l2 >= DSConfirmations
                    Rs{on1}(i, newState) = Rs{on1}(i, newState)+DSR*(l2-DSConfirmations+1);
                    Rs{on2}(i, newState) = Rs{on2}(i, newState)+DSR*(l2-DSConfirmations+1);
                end
            else % chain 2 is not orphaned
            ...
            if l2 == AD % about to enter Phase 3 -> Phase 1, chain 1 is orphaned
                if P{on1}(i, base) ~= 0
                    ...
                    Rs{on1}(i, base) = Rs{on1}(i, base)*P{on1}(i, base)+a2*beta;
                ...
                if P{on2}(i, base) ~= 0
                    ...
                    Rs{on2}(i, base) = Rs{on2}(i, base)*P{on2}(i, base)+alpha*(a2+1)+beta*a2;
                    P{on2}(i, base) = P{on2}(i, base)+alpha+beta;
                else
                    Rh{on2}(i, base) = alphaInAB*(l2-a2)+betaInAB*(l2+1-a2);
                    Rs{on2}(i, base) = alphaInAB*(a2+1)+betaInAB*a2;
                    P{on2}(i, base) = alpha+beta;
                end
                if l1 >= DSConfirmations
                    Rs{on1}(i, base) = Rs{on1}(i, base)+DSR*(l1-DSConfirmations+1);
                    Rs{on2}(i, base) = Rs{on2}(i, base)+DSR*(l1-DSConfirmations+1);
                end
            else
